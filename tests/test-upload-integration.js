...
async function checkSessionFeedback(userId) {
  const res = await fetch(`${CONTENT_STORE_URL}/files/session`, {
    method: 'GET',
  ...
  assert(Array.isArray(data.files), 'Files not an array');
  assert(data.files.length >= 0, 'Files array should exist');
}
